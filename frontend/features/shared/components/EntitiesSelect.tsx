...
const EntitiesSelect = ({
  ...
  const [selectedEntityId, setSelectedEntityId] = useState<string>(value ?? "");

  // Keep internal state in sync when controlled value changes
  useEffect(() => {
    if (typeof value === "string" && value !== selectedEntityId)
  ...
  const { data, isLoading, isFetching } = useGetFarmsDropdown({
    ...
    farmId: selectedEntityId || undefined,
  });
  const farms: FarmOption[] = useMemo(() => data?.data || [], [data]);

  const selectedFarmName = useMemo(() => {
    if (!selectedEntityId) return "";
    return farms.find((f) => f._id === selectedEntityId)?.name || "";
  }, [selectedEntityId, farms]);
                  ...
                  farms.map((farm) => (
                    <CommandItem
                      ...
                      onSelect={(currentValue) => {
                        const next =
                          currentValue === selectedEntityId ? "" : currentValue;
